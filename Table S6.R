...
for(i in c(1:(p1+p2))){for(j in c(1:(p1+p2))){Sigma[i,j]=kappa**(abs(i-j));}}
  ...
  mento=function(it){
    ...
    for(j in 1:n){
      ...
      if(modelnumber==2){Y[j]=sum(X[,j])+sum(Z[2,j])+e[j];}
      if(modelnumber==3){Y[j]=sum(X[,j])+sum(Z[2,j])+e[j]*2*pnorm(xtarget[1]);}
    }
    X1=X;rm(X);
    Z1=Z[1,,drop=FALSE];rm(Z);
    Y1=Y;rm(Y);
  ...
  #  ture value and mis value of theta
  if(scorenumber==2){
    ...
    mis_theta=1/(SigmaZonX[2,2]+sigma*sigma);
    
    denominator=SigmaZonX[2,2]+sigma*sigma;
    numerator=SigmaZonX[2,2]-SigmaZonX[2,1]*SigmaZonX[1,2]/SigmaZonX[1,1]+sigma*sigma;
    true_theta=numerator/denominator;
  } 

  output=c(round(mean( thetahat_mento - true_theta, na.rm=TRUE ), 4),
           round(sd( thetahat_mento - true_theta, na.rm=TRUE ), 4),
           round(mean( thetahat_mento - mis_theta, na.rm=TRUE ), 4),
           round(sd( thetahat_mento - mis_theta, na.rm=TRUE ), 4)
           );
  cat(output," ");
  if(kappa==0.8){cat("\n");}
