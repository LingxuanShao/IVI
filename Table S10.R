...
for(p2 in c(1:2)){
  ...
  mento=function(it){
    ...
    thetahat_on_xtarget_alternative=( alphahat3_on_xtarget/gammahat3_on_xtarget + alphahat4_on_xtarget/gammahat4_on_xtarget )/2;
   
     ### output
    return(as.list(c(thetahat_on_xtarget,thetahat_on_xtarget_alternative)));
  }#end of mento
  ...
  thetahat_mento_alternative=array(0,itermax);
  for(it in 1:itermax){
    thetahat_mento[it]=result[[2*it-1]];
    thetahat_mento_alternative[it]=result[[2*it]];
  }
  ...
  MAE_alternative=abs(thetahat_mento_alternative-true_theta_on_xtarget);
