...
obtain_ghat=function(n,X,Z,Y,h,x0,z0){
  n=dim(X)[2];
    numerator=sum(as.numeric(apply(Ker(X-x0,h),2,prod))*apply(Ker(Z[1:q2,,drop=FALSE]-z0[1:q2],h),2,prod)*apply(Z[(1+q2):p2,,drop=FALSE]==z0[(1+q2):p2],2,prod)*Y, na.rm=TRUE);
  denominator=sum(as.numeric(apply(Ker(X-x0,h),2,prod))*apply(Ker(Z[1:q2,,drop=FALSE]-z0[1:q2],h),2,prod)*apply(Z[(1+q2):p2,,drop=FALSE]==z0[(1+q2):p2],2,prod), na.rm=TRUE);
...
itermax=1000;
for(N in c(6000,9000,12000)){
for(scorenumber in c(1:2)){ # 1 for uniform and 2 for Normal
for(p1 in c(3:6)){
for(modelnumber in c(1:2)){ # 1 for Y=X, 2 for Y=X+Z, 3 for Y=X+Z+Phi
for(p2 in c(3:5)){

q1=1;q2=1;
xtarget=c(array(0,q1));
          
A=runif(10**7,-1,1);
Knorm=(mean(Ker(A,1)*Ker(A,1))*2)**q1; #square of L2 norm
rm(A);

tau=0.025; # confidence level 2tau
M=2;
n=N/(2*M); # sample size for ghat, dhat, alphahat and gammahat   
hmin=0.8;hmax=2; # candidates of the bandwidth
...
mento=function(it){
  ...
  if(scorenumber==1){
    ...
    for(k in (q1+1):p1){ # category type predictor
      for(j in 1:n){
        if(X[k,j]>-1){X[k,j]=1;}else{X[k,j]=2;}
      }
    }
    for(k in (q2+1):p2){
      for(j in 1:n){
        if(Z[k,j]>-1){Z[k,j]=1;}else{Z[k,j]=2;}
      }
    }
    temp=which(colSums(X[((1+q1):p1),])==(p1-q1));
    X=X[1:q1,temp,drop=FALSE];
    Z=Z[,temp,drop=FALSE];
  }
  if(scorenumber==2){
  ...
  Y5=c(Y1,Y2,Y3,Y4);
  #
  X1ex=cbind(X1,X2,X3,X4);
  Z1ex=cbind(Z1,Z2,Z3,Z4);
  Y1ex=c(Y1,Y2,Y3,Y4);
  #
  X2ex=cbind(X1,X2,X3,X4);
  Z2ex=cbind(Z1,Z2,Z3,Z4);
  Y2ex=c(Y1,Y2,Y3,Y4);
  #
  X3ex=cbind(X1,X2,X3,X4);
  Z3ex=cbind(Z1,Z2,Z3,Z4);
  Y3ex=c(Y1,Y2,Y3,Y4);
  #
  X4ex=cbind(X1,X2,X3,X4);
  Z4ex=cbind(Z1,Z2,Z3,Z4);
  Y4ex=c(Y1,Y2,Y3,Y4);
  ...
  fXtilde_on_xtarget=obtain_fX(dim(X5)[2],X5,Z5,Y5,h_dhat,xtarget)*(dim(X5)[2]/N);
  ...
  #V2tilde on x_target, similar formula to dhat
  ...
  #confidence interval
  radius=qnorm(1-tau,0,1)*sqrt(Vtildet_on_xtarget/(N*(h2**q1)));
...
if(modelnumber==2){
  if(scorenumber==1){
    sigma2temp=(sqrt(3)+1)*1/(2*sqrt(3))+(sqrt(3)-1)*4/(2*sqrt(3)) - ((sqrt(3)+1)*1/(2*sqrt(3))+(sqrt(3)-1)*2/(2*sqrt(3)))**2;
    true_theta_on_xtarget=(sigma*sigma)/(q2+(p2-q2)*sigma2temp+sigma*sigma);
  }
  if(scorenumber==2){
    #Calculate Var(1Z|X=x)
    NI=10**7;
    XZ=t(rmvnorm(NI, as.numeric(array(0,p1+p2)), Sigma));
    ...
    true_theta_on_xtarget=(sigma*sigma)/(var(colSums(Z))+sigma*sigma);
  }
}
