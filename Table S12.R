...
#linear regression
linear=function(X,Z,Y,x0,order){
  n=dim(X)[2];
  D=array(0,c(n,(order+1)));
  for(i in 1:n){
    D[i,1]=1;
    D[i,2]=X[1,i];
    if(order==2){D[i,3]=Z[1,i];}
  }
  return(as.numeric(c(1,x0)%*%solve(t(D)%*%D)%*%t(D)%*%Y))
}
...
for(xtarget in c(-0.5,0,0.5)){
for(modelnumber in c(1:2)){ # 1 for Y=X+Z, 2 for Y=cos(X)+cos(Z)
scorenumber=1;
p1=1;
p2=1;
...
scaling=1
  ...
  mento=function(it){
    ...
    Y=array(0,n);
    for(j in 1:n){
      if(modelnumber==1){Y[j]=sum(X[,j])+sum(Z[,j])+e[j];}
      if(modelnumber==2){Y[j]=sum(cos(2*pi*X[,j]/sqrt(3)))+sum(cos(2*pi*Z[,j]/sqrt(3)))+sqrt(1+scaling*X[,j]*X[,j])*e[j];}
    }
    X1=X;rm(X);
    Z1=Z;rm(Z);
    Y1=Y;rm(Y);
    ...
    ######### local mean regression for the first stage
    # data1ex for ghat
    ...
    ### local mean for the second stage
    ...
    ### linear for the second stage
    alphahat3_on_xtarget=linear(X2,Z2,(Y2-ghat1)*(Y2-ghat1),xtarget,1);
    alphahat4_on_xtarget=linear(X1,Z1,(Y1-ghat2)*(Y1-ghat2),xtarget,1);
    gammahat3_on_xtarget=linear(X4,Z4,(Y4-dhat3)*(Y4-dhat3),xtarget,1);
    gammahat4_on_xtarget=linear(X3,Z3,(Y3-dhat4)*(Y3-dhat4),xtarget,1);
    thetahat_ml_on_xtarget=(alphahat3_on_xtarget+alphahat4_on_xtarget)/(gammahat3_on_xtarget+gammahat4_on_xtarget);
    
    
    ######### linear regression for the first stage
    # data1ex for ghat
    ghat1=array(0,dim(X2)[2]); # on (X2,Z2)
    for(j in 1:dim(X2)[2]){
      ghat1[j]=linear(X1ex,Z1ex,Y1ex,c(X2[,j],Z2[,j]),2);
    }
    # data2ex for ghat
    ghat2=array(0,dim(X1)[2]); # on (X1,Z1)
    for(j in 1:dim(X1)[2]){
      ghat2[j]=linear(X2ex,Z2ex,Y2ex,c(X1[,j],Z1[,j]),2);
    }
    # data3ex for dhat
    dhat3=array(0,dim(X4)[2]); # on (X4,Z4)
    for(j in 1:dim(X4)[2]){
      dhat3[j]=linear(X3ex,Z3ex,Y3ex,X4[,j],1);
    }
    # data4ex for dhat
    dhat4=array(0,dim(X3)[2]); # on (X3,Z3)
    for(j in 1:dim(X3)[2]){
      dhat4[j]=linear(X4ex,Z4ex,Y4ex,X3[,j],1);
    }
    ...
    gammahat4_on_xtarget=obtain_gammahat(dim(X3)[2],X3,Z3,Y3,h_gammahat4,xtarget,dhat4);
    # thetahat
    thetahat_lm_on_xtarget=(alphahat3_on_xtarget+alphahat4_on_xtarget)/(gammahat3_on_xtarget+gammahat4_on_xtarget);
    ...
    gammahat3_on_xtarget=linear(X4,Z4,(Y4-dhat3)*(Y4-dhat3),xtarget,1);
    gammahat4_on_xtarget=linear(X3,Z3,(Y3-dhat4)*(Y3-dhat4),xtarget,1);
    thetahat_ll_on_xtarget=(alphahat3_on_xtarget+alphahat4_on_xtarget)/(gammahat3_on_xtarget+gammahat4_on_xtarget);
    
    ### output
    return(as.list(c(thetahat_on_xtarget,thetahat_ml_on_xtarget,thetahat_lm_on_xtarget,thetahat_ll_on_xtarget)));
  }#end of mento
  ...
  thetahat_ml_mento=array(0,itermax);
  thetahat_lm_mento=array(0,itermax);
  thetahat_ll_mento=array(0,itermax);
  for(it in 1:itermax){
    thetahat_mento[it]   =result[[4*it-3]];
    thetahat_ml_mento[it]=result[[4*it-2]];
    thetahat_lm_mento[it]=result[[4*it-1]];
    thetahat_ll_mento[it]=result[[4*it-0]];
  }
  
  # ture value of theta
  if(modelnumber==1){true_theta_on_xtarget=1/2;}
  if(modelnumber==2){true_theta_on_xtarget=(1+scaling*xtarget*xtarget)/(1.5+scaling*xtarget*xtarget);}

  bias   =(thetahat_mento   -true_theta_on_xtarget);
  bias_ml=(thetahat_ml_mento-true_theta_on_xtarget);
  bias_lm=(thetahat_lm_mento-true_theta_on_xtarget);
  bias_ll=(thetahat_ll_mento-true_theta_on_xtarget);
  
  output=c(round(mean(bias,na.rm=TRUE),4),
           round(sd(bias,na.rm=TRUE),4),
           round(mean(bias_ml,na.rm=TRUE),4),
           round(sd(bias_ml,na.rm=TRUE),4),
           round(mean(bias_lm,na.rm=TRUE),4),
           round(sd(bias_lm,na.rm=TRUE),4),
           round(mean(bias_ll,na.rm=TRUE),4),
           round(sd(bias_ll,na.rm=TRUE),4)
           );
  cat(output," ");
  if((modelnumber==2)){cat("\n");}
