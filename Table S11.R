...
mento=function(it){
  ...
  thetastarhat_on_xtarget_alternative=(alphahat3_on_xtarget/gammahat3_on_xtarget+alphahat4_on_xtarget/gammahat4_on_xtarget)/2;
  
  ### output
  return(as.list(c(thetastarhat_on_xtarget,thetastarhat_on_xtarget_alternative)));
}#end of mento
...
thetastarhat_mento=array(0,itermax);
thetastarhat_alternative_mento=array(0,itermax);
for(it in 1:itermax){
  thetastarhat_mento[it]=result[[2*it-1]];
  thetastarhat_alternative_mento[it]=result[[2*it]];
}
...
MAE=abs(thetastarhat_mento-true_theta_on_xtarget);
MAE_alternative=abs(thetastarhat_alternative_mento-true_theta_on_xtarget);

output=c(N,scorenumber,xtarget_number,modelnumber,
         round(mean(MAE,na.rm=TRUE),4),
         round(sd(MAE,na.rm=TRUE),4),
         round(mean(MAE_alternative,na.rm=TRUE),4),
         round(sd(MAE_alternative,na.rm=TRUE),4));
cat(output,"\n");
